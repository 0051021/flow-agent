...
>
> 子文档索引：
> - FlowAgent PRD（功能细节）→ [`FlowAgent产品说明.md`](./FlowAgent产品说明.md)
> - 产品矩阵（四件套定位）→ [`产品矩阵.md`](./产品矩阵.md)
> - 产品战略（知识容器、Gene 飞轮）→ [`产品策略.md`](./产品策略.md)
> - 上下文架构（六层模型、Context Bus）→ [`上下文架构.md`](./上下文架构.md)
> - Skill 平台架构（切割原则、双形态）→ [`Skill架构思考.md`](./Skill架构思考.md)
> - Prompt 架构 → [`提示词架构.md`](./提示词架构.md)
> - 代码级技术架构 → [`FlowAgent架构说明.md`](./FlowAgent架构说明.md)
...
> 完整设计见 [`Skill架构思考.md`](./Skill架构思考.md)。
...
> 功能细节见 [`FlowAgent产品说明.md`](./FlowAgent产品说明.md)。
...
| 状态 | 说明 |
|------|------|
| 原则已定义 | 数据隔离、最小暴露、审计可追溯（`FlowAgent产品说明.md` 2.4） |
...
> 完整设计见 [`FlowAgent产品说明.md`](./FlowAgent产品说明.md) 3.2.7 和 4.4。
...
| 决策 | 理由 | 出处 |
|------|------|------|
| 自建执行层，不依赖 Dify/LangGraph | 数据闭环（运行数据直接反哺知识）+ 配置外化（按产品需求设计）+ 演进自由度 | `产品矩阵.md` 3.1 |
| Skill = 最小业务验收单元 | 业务方能看懂输出、能判断对不对，而非最小技术单元 | `Skill架构思考.md` 第三章 |
| Gene 双形态（Human Doc + Model Gene） | EvoMap 实验表明完整文档注入模型反而降低性能，~230 token Gene 稳定胜出 | `Skill架构思考.md` 5.3 |
| Context Package 携带 quality_signals | 防止级联失败——上游质量不够时在交接点拦截，不等到下游失败 | `上下文架构.md` 第四章 |
| 参数可以热调，结构必须版本化 | Layer 1-2 热更新不违反版本控制，参数调整有独立审计轨迹 | `FlowAgent产品说明.md` 3.2.7 |
| 方案蓝图定位（不直接执行） | AI 生成工作流 5 步成功率仅 59%，10 步降至 35%；必须有人审核 | `FlowAgent产品说明.md` 4.1 |

### 8.2 执行模型决策

| 决策 | 理由 | 出处 |
|------|------|------|
| Hybrid 双引擎（Workflow + Agentic） | 80% 流程需要确定性，20% 需要灵活性，纯 Agentic 不可控 | AOI 第七章 + `FlowAgent产品说明.md` 3.2.1 |
| LLM 不拥有最终执行权 | 企业级场景必须有约束边界，Constrained Planning | AOI 4.3 |
| Workflow 的 Skill 是绑定，Agentic 的 Skill 是授权 | 两种关系的用户决策、决定时机、Agent 自主性完全不同 | `Skill架构思考.md` 第十章 |
| 三种人机协作模式 | Human in the loop（节点级）/ on the loop（策略级）/ Intervention（参数级）覆盖全场景 | `FlowAgent产品说明.md` 2.2.2 |

### 8.3 产品决策

| 决策 | 理由 | 出处 |
|------|------|------|
| 当前阶段核心用户是技术方 | AI 方案不保证正确，业务方缺乏判断力，技术方能纠错 | `FlowAgent产品说明.md` 1.1 |
| 反问不拦截，改为标记提醒 | 用户没看到方案就被迫回答细节问题，缺乏掌控感 | `FlowAgent产品说明.md` 1.1.2 |
| 双角色协作 | 企业现实是业务方和技术方两拨人，信息鸿沟是最大瓶颈 | `FlowAgent产品说明.md` 13.2 |
| 知识容器是核心壁垒 | 工具能力可替代，沉淀的企业私有知识不可替代 | `产品策略.md` 第一章 |

### 8.4 上下文与数据决策

| 决策 | 理由 | 出处 |
|------|------|------|
| 六层上下文模型 | 不同层级生命周期、作用域、消费者完全不同，不能一锅煮 | `上下文架构.md` 第二章 |
| Gene 注入总预算 ≤ 500 token | 控制 LLM 推理预算，优先级：通用 > 组合 > 场景 > Skill | `上下文架构.md` 9.3 |
| 组合 Gene（v13 新增） | 跨节点失败比单节点失败更难发现和修复，需要专门的经验沉淀 | `上下文架构.md` 第五章 |
| 质量传导层默认策略 warn | 已有人工确认节点兜底；不可逆操作的交接用 strict | `上下文架构.md` 9.2 |
...
架构全景.md          ← 你在这里（架构北极星）
    ...
    ├── 产品矩阵.md          四件套定位和关系
    │
    ├── FlowAgent产品说明.md       FlowAgent PRD（功能细节）
    │     └── 提示词架构.md    Prompt 设计
    │
    ├── 上下文架构.md    上下文架构（六层模型 + 流动 + 压缩）
    │
    ├── 产品策略.md        知识容器 + Gene 进化 + 竞争壁垒
    │
    ├── Skill架构思考.md   Skill 平台架构
    │
    └── FlowAgent架构说明.md  代码级技术架构
```
...
1. 本文档（全景理解）
2. `产品矩阵.md`（四件套关系）
3. `FlowAgent产品说明.md`（功能细节）
4. `上下文架构.md`（上下文深度设计）
5. `Skill架构思考.md`（Skill 深度设计）
6. `产品策略.md`（战略思考）
