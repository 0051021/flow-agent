...
```text
用户上传 PPTX / 流程图 PDF / Word / 图片
→ Job 文件池保存原文件
→ 具备工具能力的 Agent 根据文件分类和任务目标选择读取方式
→ 读取页面标题、正文、备注、表格、图形/箭头关系
→ 提取业务流程线索
→ 如果原文是英文，整理成中文业务流程图
```
...
- quick brief 只是索引和路标，不代表完整文件内容。
- PPTX 中如果包含流程图，优先还原图中的节点、顺序、分支、角色和产物。
- 如果 Agent 无法确定图形关系，不要臆造，应生成追问。
...
| `review_feedback` | 检查、返修、回填相关节点 | 检查证书、错误返修、回填有效期；如果存在条件分支，应拆成多个节点 |
...
注意：判断对象是“每个节点”，不是整个 Job。一个固定申请流程中也可能包含业务判断节点；一个目标导向的 RCA、运营、售后任务中也可能包含固定操作节点。业务侧不展示 workflow / agentic 术语，只展示节点语义。
...
- 判断目标：这一步要决定什么。
- 关键判断依据：做判断时看哪些资料、信号、指标或上下文。
- 判断规则：如何根据依据做判断，什么情况输出什么结论或进入什么分支。
- 输出结果：判断完成后给下一步使用的结论字段、分支或建议。
- 需要人工处理的情况：原人工流程里哪些情况要停下来、补材料、找主管/负责人确认、转给其他人，或暂时不能下结论。
- 完成标准：这一步达到什么验收状态才可以停止并进入下一步。
...
6. 接收 IMI 证书
7. 检查证书内容
8. 证书有错：反馈错误字段和正确值，并等待重新出证
9. 证书无误：回填证书编号、有效期并归档 IMI 大表
```

注意：不要生成“证书校对存档”这类混合节点。校对只是判断/检查；只有证书无误后才会回填和归档。证书有错时不会归档，而是进入反馈错误、等待重出证、重新校对的闭环。
