...
FlowAgent 的所有架构决策遵循以下原则（详见 `FlowAgent产品说明.md` 第二章）：
...
当前处于阶段一（详见 `FlowAgent产品说明.md` 第 1.2 节），核心用户是**技术方**。业务方口述需求 → 技术方在 FlowAgent 上生成并修正方案 → 确认后参考蓝图搭建。确认后的方案自动沉淀为知识容器的一部分。
...
技术方通过 FlowAgent 生成并修正方案，确认后参考蓝图在 Dify/LangGraph 等平台搭建。同时，每次确认都在沉淀知识（见 `FlowAgent产品说明.md` 第五章）。
...
> v4 的强制反问卡片在 v6 中被替换为标记提醒 + 按需查看。详见 `FlowAgent产品说明.md` 第 1.1.2 节。
...
> 详见 `FlowAgent产品说明.md` 第七章。v6 细化了 UI 形式（澄清卡片）和完整串联流程。
...
> 详见 `FlowAgent产品说明.md` 第五章。知识容器是产品从"工具"变为"平台"的关键。

> v12 更新：知识容器采用双形态存储（Human Doc + Model Gene），详见 `产品策略.md` 和 `Skill架构思考.md`。
...
**知识容器存储的五种数据**（详见 `FlowAgent产品说明.md` 第 5.2 节）：
...
> 详见 `FlowAgent产品说明.md` 第 5.7 节。此处记录技术实现架构。
...
> 详见 `FlowAgent产品说明.md` 第 1.1.3 节。待确认问题集不是新系统，而是批注系统的适用范围扩展。
...
> 详见 `FlowAgent产品说明.md` 第六章。Skill 平台是独立产品，此处仅记录 FlowAgent 侧的集成架构。
...
> v10 更新：明确 FlowAgent 输出为方案蓝图（非可执行代码），试运行定位为业务体验预览。详见 `FlowAgent产品说明.md` 第四章。
...
**Skill 标准化是自动化程度的关键变量**（Skill 平台详见 13.7 节和 `FlowAgent产品说明.md` 第六章）：
...
> 详见 `FlowAgent产品说明.md` 第八章。知识面板从静态文件展示升级为支持 AI 代码生成 + 沙箱执行的数据工作台。
...
> 详见 `FlowAgent产品说明.md` 第 3.5-3.6 节、第 4.4-4.5 节。
