...
对于 PPTX、流程图 PDF、Word、图片、录屏转写等复杂材料，推荐链路是：
...
→ Agent 收到任务和文件分组
→ Agent 自己选择读取/解析方式
→ Agent 提取文本、页面结构、流程箭头、图形关系和备注
→ Agent 把英文或混合材料整理成中文业务流程线索
→ 生成业务流程澄清稿
```

这里的关键不是“平台先把文件全文预处理完”，而是“平台把 Job 文件池和读取规则交给 Agent，让 Agent 对需要的材料做按需理解”。
...
业务流程澄清稿要把真实人工流程中的条件走向画出来，而不是把条件塞进一个节点描述里。

当业务描述或流程文件出现以下表达时，优先拆成分支：
...
→ 判断 MSDS 是否存在
   ├─ 已存在 → 打印盖章 MSDS
   └─ 缺失 → 邮件联系品控补 MSDS → 打印盖章 MSDS
```

同样，检查/判断节点不能同时承载成功分支才会发生的产出。

例如：

```text
校对 IMI 证书，证书有错就把错误字段和正确值发给代理/海关；证书无误才回填证书编号、有效期并归档。
```

不应该生成成“证书校对存档”一个节点。应该拆为：

```text
校对 IMI 证书
├─ 证书无误 → 回填证书编号和有效期 → 归档 IMI 大表/IMI List
└─ 证书有错 → 邮件反馈错误字段和正确值 → 等待重新出证 → 回到校对 IMI 证书
```

规则：

- 检查/判断节点的输出是“检查结果、差异清单、是否通过”，不是“已归档、已回填”。
- 成功分支产物只能放在成功分支节点里。
- 失败分支需要写出真实人工处理动作，并在必要时回到前面的等待或检查节点。
...
9. 如果一句话里包含“如果/缺失则/有错则/不一致则/退回则”等条件表达，优先拆成判断节点、条件分支和处理节点，不要压进单个节点描述。
10. 如果文件是 PPTX 或流程图类材料，必须读取原文件内容和页面结构；quick brief 只能作为索引，不代表完整上下文。
11. 如果原文是英文，最终节点名称、说明、输入输出、判断规则和追问都使用中文。
...
- 不要把“缺失则联系谁补”“有错则返回谁修改”“无错则继续”等条件路径写成一句描述，应该画成分支。
...
4. 如果某个节点里出现“如果/缺失则/有错则/不一致则/退回则”等条件表达，但图上没有对应分支，优先追问这条条件路径应该怎么走。
5. 如果问题可以从上传材料中读到，不要再问业务方。
6. 如果只是字段文案不够漂亮，不要提问。
7. 问题要用业务语言，不要出现 workflow、agentic、schema、executionMode、Skill、JobSpec 等技术词。
8. 每个问题尽量提供 2-3 个选项，方便业务方点选。
...
如果当前流程写成“证书校对存档”，Review 应该指出这不是一个清晰节点：

```json
{
  "severity": "high",
  "type": "mixed_check_and_success_output",
  "targetNodeIds": ["node-7"],
  "title": "证书检查和成功归档被合在同一个节点",
  "description": "当前节点同时包含证书校对、证书无误后的回填归档，以及证书有错后的返修处理，成功和失败路径没有拆开。",
  "whyItMatters": "证书有错时不会归档，而是需要反馈错误并等待重新出证；如果合并在一个节点里，业务流程会误导后续技术方案。",
  "questionToBusiness": "证书核对不一致时，是邮件反馈错误字段并等待重新出证，然后重新校对；证书无误时才回填并归档吗？",
  "suggestedOptions": [
    "是，错误反馈后等待重出证，再回到证书校对",
    "不是，错误时还有其他处理路径",
    "不确定，需要补充说明"
  ]
}
